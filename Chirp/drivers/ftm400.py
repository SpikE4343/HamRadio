...
UPLOAD_HEADER = "\x66\x08\x42\x42\x42\x42\x42\x42\x42\x42\x42\x42\x42\x42\x42\x42\x42\xFE"
    
def _clone_out(radio):
    radio.pipe.setTimeout(1)

    
    #for byte in UPLOAD_HEADER:
    radio.pipe.write(UPLOAD_HEADER)
        
    ack = radio.pipe.read(1)
    
    #f = open( 'ftm-400-u.dump', 'w')
    #f.write( ack );
    #f.close()
    # Seriously, WTF Yaesu?
    ranges = [
    ...
    for start, end in ranges:
        for i in range(start, end+1, 128):
            ...
            ack = radio.pipe.read(1)
            if ack != "\x06":
                raise errors.RadioError("Radio refused block %i, %x" % (i / 128, ack))
            time.sleep(0.05)
